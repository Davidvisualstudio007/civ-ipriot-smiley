...
class SenseHat:
    def __init__(self, background=DEFAULT_RGB):
        ...
        # --- CHANGED: use threading + queue.Queue (not multiprocessing) ---
        self.queue = queue.Queue()
        self._gui_thread = threading.Thread(
            target=self.run_hat_gui, args=(background,)
        )
        self._gui_thread.start()
        # Give the GUI a moment to spin up (keeps behavior similar to original)
        time.sleep(0.2)

    def run_hat_gui(self, initial_rgb_values):
        """tkinter GUI for the mock SenseHAT. All tkinter calls stay on this thread."""
    ...
    def refresh_gui_from_queue(self):
        """Updates the GUI using a queue for communication with the main thread."""
        ...
        self.root.after(100, SenseHat.refresh_gui_from_queue, self)
    ...
    def create_led_matrix(self):
        ...
        for i, j in itertools.product(range(8), range(8)):
            frame = tk.Frame(self.root, width=30, height=30, bd=1, relief="solid")
    ...
    def _set_pixels(self, rgb_values):
        ...
        if self.low_light:
            ...
            rgb_values = [
                (int(r * dimming_factor), int(g * dimming_factor), int(b * dimming_factor))
                for r, g, b in rgb_values
            ]

        for i, rgb in enumerate(rgb_values):
            # safeguard against invalid pixel data
            if callable(rgb):
                self.logger.warning(f"Invalid RGB value (method found): {rgb}")
                continue
            if not isinstance(rgb, (list, tuple)) or len(rgb) != 3:
                self.logger.warning(f"Invalid RGB value: {rgb}")
                continue
            r, g, b = rgb
            self.led_matrix[i].config(bg="#%02x%02x%02x" % (int(r), int(g), int(b)))  # type: ignore
    ...
    def close(self):
        # Gracefully close the tkinter window; thread exits when mainloop ends
        try:
            self.root.destroy()
        except Exception:
            pass
...
def main():  # sourcery skip: extract-duplicate-method
    ...
    # Keep the main thread alive for a bit while the GUI is visible
    time.sleep(60)
